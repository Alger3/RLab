...
### 2. Write down the distribution function 𝐹 of 𝑋 
$F_X(x)=
\begin{cases}
0,& \quad if \quad x<0 \\
...
$\int_{-\infty}^{\infty}p(x)dx=\int_{-\infty}^{\infty}0dx+\int_{-\infty}^{\infty}\lambda e^{-\lambda x} dx$
$Then, \quad \int_{-\infty}^{\infty}\lambda e^{-\lambda x}dx=[-e^{-\lambda x}]_0^{\infty}=(0-(-1))=1$
...
## 3.2 Q4
$\mathbb{E}(X)=\int_{-\infty}^\infty xp_{\lambda}(x)dx=\int_{0}^\infty x \lambda e^{-\lambda x}dx=1/\lambda$
...
ggplot() + labs(x="x",y="Probability") + theme_bw() + geom_line(data=gaussian_df, aes(x,y=pdf,color="Gaussian pdf"), size=2) + geom_col(data=binom_df, aes(x=x,y=pmf,color="Binomial pmf", fill="Binomial pmf")) + scale_color_manual(name="myLegend", values=colors) + scale_fill_manual(name="myLegend",values=fill)+xlim(c(20,50))
```
